...
string findNonRepeatingCharOutputUsingDoublyEndedQueue(string charStream)
{
    ...
    for (auto character : charStream)
    {
        ...
        while (dq.empty() == false && foundNonRepeatingCharacter == false)
        {
            ...
            {
                // cout << "Popped: " << frontCharacter << endl;
                dq.pop_front();
            }
...
int main()
{
    ...
    for (int i = 0; i < testCases; i++)
    {
        cout << "For _" << charStreams[i] << "_ -> " << findNonRepeatingCharOutputUsingDoublyEndedQueue(charStreams[i]) << endl;
    }
